...
class ConfigFileRepositoryImpl @Inject constructor(
    ...
    override suspend fun saveConfiguration(uri: Uri, configData: ConfigData): Result<Unit> =
            ...
            runCatching {
                context.contentResolver.openOutputStream(uri)?.bufferedWriter()?.use { writer ->
                    val content = buildString {
                        ...
                        appendLine("LAMPON=${if (configData.lampPosOn == GateStatus.START) "OPENING" else "OPENED"}")
                        appendLine("LAMPOFF=${if (configData.lampPosOff == GateStatus.START) "CLOSING" else "CLOSED"}")
                        appendLine("LEDOPEN=${configData.ledOpenColor.name}")
                        appendLine("LEDOPENPOS=${if (configData.ledOpenPos == GateStatus.START) "OPENING" else "OPENED"}")
                        appendLine("LEDCLOSE=${configData.ledCloseColor.name}")
                        appendLine("LEDCLOSEPOS=${if (configData.ledClosePos == GateStatus.START) "CLOSING" else "CLOSED"}")
    ...
    override suspend fun loadConfiguration(uri: Uri): Result<Map<String, String>> =
        withContext(Dispatchers.IO) {
            runCatching {
